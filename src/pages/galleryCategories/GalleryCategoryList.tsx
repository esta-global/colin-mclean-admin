...
export function GalleryCategoryList() {
  ...
  useEffect(
    function () {
      async function getData() {
        setLoading(true);
        let url = `/galleryCategories?page=${pagination.page}&limit=${pagination.limit}`;
          ...
          return (
            <div className="d-flex gap-1">
              <Link
                ...
                to={{
                  pathname: `/galleryCategories/edit/${value}`,
                }}
              >
  ...
  async function handleDeleteData(recordId: string | string[]) {
    ...
    if (Array.isArray(recordId)) {
      apiResponse = await remove(`/galleryCategories`, recordId);
    } else {
      apiResponse = await remove(`/galleryCategories/${recordId}`);
    }
        ...
        <div className="col-md-12 grid-margin">
          <div className="d-flex justify-content-between align-items-center">
            <div className="d-flex gap-2">
              <GoBackButton />
              <h4 className="font-weight-bold mb-0">Gallery Category List</h4>
            </div>
            <div>
              <Link
                to={"/galleryCategories/add"}
                type="button"

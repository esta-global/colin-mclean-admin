...
export function FaqCategoryList() {
  ...
  useEffect(
    function () {
      async function getData() {
        setLoading(true);
        let url = `/faqsCategory?page=${pagination.page}&limit=${pagination.limit}`;
          ...
          return (
            <div className="d-flex gap-1">
              <Link
                ...
                to={{
                  pathname: `/faqsCategory/edit/${value}`,
                }}
              >
  ...
  async function handleDeleteData(recordId: string | string[]) {
    ...
    if (Array.isArray(recordId)) {
      apiResponse = await remove(`/faqsCategory`, recordId);
    } else {
      apiResponse = await remove(`/faqsCategory/${recordId}`);
    }
        ...
        <div className="col-md-12 grid-margin">
          <div className="d-flex justify-content-between align-items-center">
            <div className="d-flex gap-2">
              <GoBackButton />
              <h4 className="font-weight-bold mb-0">FaQs Category List</h4>
            </div>
            <div>
              <Link
                to={"/faqsCategory/add"}

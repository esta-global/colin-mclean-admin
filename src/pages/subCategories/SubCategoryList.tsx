...
export function SubCategoryList() {
  ...
  useEffect(
    function () {
      async function getData() {
        setLoading(true);
        let url = `/subCategories?page=${pagination.page}&limit=${pagination.limit}`;
          ...
          return (
            <div className="d-flex gap-1">
              <Link
                ...
                to={{
                  pathname: `/subCategories/edit/${value}`,
                }}
              >
  ...
  async function handleDeleteData(recordId: string | string[]) {
    ...
    if (Array.isArray(recordId)) {
      apiResponse = await remove(`/subCategories`, recordId);
    } else {
      apiResponse = await remove(`/subCategories/${recordId}`);
    }
        ...
        <div className="col-md-12 grid-margin">
          <div className="d-flex justify-content-between align-items-center">
            <div className="d-flex gap-2">
              <GoBackButton />
              <h4 className="font-weight-bold mb-0">Sub Category List</h4>
            </div>
            <div>
              <Link

...
export function CategoryShowcaseList() {
  ...
  useEffect(
    function () {
      async function getData() {
        setLoading(true);
        let url = `/categoryShowcase?page=${pagination.page}&limit=${pagination.limit}`;
...

      {
        ...
        Cell: ({ value }: any) => {
          if (value) {
            return (
              <div>
                <img className="img" src={addUrlToFile(value)} />
              </div>
            );
          } else {
            return null;
          ...
          return (
            <div className="d-flex gap-1">
              <Link
                ...
                to={{
                  pathname: `/categoryShowcase/edit/${value}`,
                }}
              >
  ...
  async function handleDeleteData(recordId: string | string[]) {
    ...
    if (Array.isArray(recordId)) {
      apiResponse = await remove(`/categoryShowcase`, recordId);
    } else {
      apiResponse = await remove(`/categoryShowcase/${recordId}`);
    }
        ...
        <div className="col-md-12 grid-margin">
          <div className="d-flex justify-content-between align-items-center">
            <div className="d-flex gap-2">
              <GoBackButton />
              <h4 className="font-weight-bold mb-0">Category Showcase List</h4>
            </div>
            <div>
              <Link
                to={"/categoryShowcase/add"}
                type="button"

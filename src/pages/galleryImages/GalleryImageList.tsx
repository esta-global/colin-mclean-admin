...
export function GalleryImageList() {
  ...
  useEffect(
    function () {
      async function getData() {
        setLoading(true);
        let url = `/galleryImages?page=${pagination.page}&limit=${pagination.limit}`;
          ...
          return (
            <div className="d-flex gap-1">
              <Link
                ...
                to={{
                  pathname: `/galleryImages/edit/${value}`,
                }}
              >
  ...
  const data = React.useMemo(() => {
    return records.map((data) => {
      return {
        galleryCategory: data.galleryCategory?.name,
  ...
  async function handleDeleteData(recordId: string | string[]) {
    ...
    if (Array.isArray(recordId)) {
      apiResponse = await remove(`/galleryImages`, recordId);
    } else {
      apiResponse = await remove(`/galleryImages/${recordId}`);
    }
        ...
        <div className="col-md-12 grid-margin">
          <div className="d-flex justify-content-between align-items-center">
            <div className="d-flex gap-2">
              <GoBackButton />
              <h4 className="font-weight-bold mb-0">Gallery Image List</h4>
            </div>
            <div>
              <Link
                to={"/galleryImages/add"}
                type="button"

...
export function ShapeList() {
  ...
  useEffect(
    function () {
      async function getData() {
        setLoading(true);
        let url = `/shapes?page=${pagination.page}&limit=${pagination.limit}`;
  ...
  async function handleDeleteData(recordId: string | string[]) {
    ...
    if (Array.isArray(recordId)) {
      apiResponse = await remove(`/shapes`, recordId);
    } else {
      apiResponse = await remove(`/shapes/${recordId}`);
    }
        ...
        <div className="col-md-12 grid-margin">
          <div className="d-flex justify-content-between align-items-center">
            <div className="d-flex gap-2">
              <GoBackButton />
              <h4 className="font-weight-bold mb-0">Shape List</h4>
            </div>
            <div>
              <Link
                title="Alt + '+'"
